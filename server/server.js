...
const session = expressSession({
	...
	cookie            : {
		secure   : true,
		httpOnly : true,
		maxAge   : 24 * 60 * 60 * 1000 // Expire after 1 hour since last request from user
	}
});
...
async function runWebSocketServer()
{
	...
	io.on('connection', (socket) =>
	{
		...
		queue.push(async () =>
		{
			...
			let cpRoles = peers.get(peerId);
			let roomExisting = rooms.get(roomId);
			
			console.log("------ cpRoles:  " + cpRoles);
            let allowed = true;//Boolean(socket.handshake.session.passport) && Boolean(socket.handshake.session.passport.user);
 			const room = await getOrCreateRoom({ roomId,  allowed});
			if(room == "not_allowed"){
				console.log('must log in for room creation');

				if (socket){
					//socket.disconnect(true);
					// sending to the client
					socket.emit('error', 'You must login first?', 1, 2, 'abc');
				}
				return;
			...
			)
			{
				...
				if (typeof config.userMapping === 'function')
				{
					await config.userMapping({ peer, roomId, userinfo: _userinfo, roomExisting });
				}
 ...
 */
async function getOrCreateRoom({ roomId , allowed})
{
