...
const ROUTER_SCALE_SIZE = config.routerScaleSize || 200;
...
class Room extends EventEmitter
{
	...
	{
		logger.warn('create() [roomId:"%s"]', roomId);
	...
	async _handleSocketRequest(peer, request, cb)
	{
		...
		switch (request.method)
		{
			...
			case 'silenceRoom':
			{
				...
				// Spread to others
				this._notification(peer.socket, 'silenceRoom', {
					peerId : peer.id
				}, true);
			...
			case 'unsilenceRoom':
			{
				...
				// Spread to others
				this._notification(peer.socket, 'unsilenceRoom', {
					peerId : peer.id
				}, true);
			...
			case 'moderator:setFixedSpotlight':
			{
				if (!this._hasPermission(peer, MODERATE_ROOM))
					throw new Error('peer not authorized');
				this._fixed = request.data;

				// Spread to others
				this._notification(peer.socket, 'moderator:setFixedFilmStrip', request.data, true);

				cb();

				break;
			}
			case 'moderator:makeModerator':
				{
					if (!this._hasPermission(peer, MODERATE_ROOM))
						throw new Error('peer not authorized');
					//this._fixed = request.data;
					var newPeer = this._peers[request.data];
					newPeer.addRole(userRoles.MODERATOR);
					// Spread to others
